    ...
    <View style={[styles.activeBanner, { backgroundColor: colors.primary + "15", borderColor: colors.primary }]}>
      <View style={styles.activeBannerTop}>
        <View style={styles.activeBannerLeft}>
          ...
          <Text style={[styles.activeBannerLabel, { color: colors.primary }]}>
      ...
      <Text style={[styles.activeBannerRestaurant, { color: colors.foreground }]} numberOfLines={1}>
        {order.restaurantName}
      </Text>
      <Text style={[styles.activeBannerOrder, { color: colors.mutedForeground }]} numberOfLines={2}>
        {order.orderText}
      </Text>
      <TouchableOpacity
        style={[styles.trackBannerBtn, { backgroundColor: colors.primary }]}
        onPress={() =>
          router.push({ pathname: "/order-tracking/[id]", params: { id: order.id } })
        }
      >
        <MaterialIcons name="delivery-dining" size={18} color="#fff" />
        <Text style={styles.trackBannerBtnText}>{t("orders.track")}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
export default function OrdersTab() {
  ...
  const { t } = useTranslation();
  const { orders: contextOrders, activeOrder } = useOrders();
  ...
  const fetchPage = useCallback(async (pageNum: number, reset = false) => {
    ...
    try {
      const data = (await customFetch(
        `/api/orders?page=${pageNum}&limit=${PAGE_LIMIT}`
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 22, fontWeight: "800", textAlign: "left" },
  empty: {
    flex: 1,
    alignItems: "center",
  ...
  activeBannerLeft: { flexDirection: "row", alignItems: "center", gap: 6 },
  activeDot: { width: 8, height: 8, borderRadius: 4 },
  activeBannerLabel: { fontSize: 12, fontWeight: "700" },
  activeBannerRestaurant: { fontSize: 16, fontWeight: "800" },
  activeBannerOrder: { fontSize: 13, lineHeight: 20, textAlign: "left" },
  trackBannerBtn: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
  ...
  trackBannerBtnText: { color: "#fff", fontSize: 14, fontWeight: "700" },
